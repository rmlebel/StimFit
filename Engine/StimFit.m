...
opt.RFr.angle = opt.RFr.angle(:)';
if length(opt.RFr.angle) ~= opt.etl
    error('Specified refocusing angles in opt.RFr.angle do not match the echo train length opt.etl');
end
