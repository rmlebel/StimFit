...
if strcmpi(opt.mode,'s')
    opt.Dz  = [0 0.5];      %   Spatial bounds (cm) (should exceed slice thickness, can start at 0 for half profile)
    opt.Nz  = 25;           %   Number of positions to simulate across slice
...
if strcmpi(opt.mode,'s')
    opt.RFe.path{1} = '';   %   Path to excitation waveform (empty for GUI)
...
if strcmpi(opt.mode,'s')
    opt.RFr.path{1} = '';   %   Path to refocusing waveform (empty for GUI)
...
opt.lsq.Icomp.XU   = [3.000 1e+6 1.00];      %   Upper bound (1 x 3)
...
opt.lsq.IIcomp.XU  = [0.250 1e+6 3.000 1e+6 1.20];    %   Upper bound (1 x 5)
...
opt.lsq.IIIcomp.XU = [0.035 1e+6 0.130 1e+6 3.000 1e+6 1.20];  %   Upper bound (1 x 7)
